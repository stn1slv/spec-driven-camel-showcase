...
- [ ] T004 Configure Camel REST DSL to use servlet component with json binding in `src/main/java/com/example/api/RestConfig.java`
- [ ] T005 [P] Setup global `onException` handler for RFC 9457 Problem Details formatting in `src/main/java/com/example/routes/GlobalErrorRoute.java`
...
- [ ] T006 [P] [Flow1] Create Camel Route integration test utilizing "Advice Once" pattern (no `@DirtiesContext`) in `src/test/java/com/example/integration/BasePriceRouteTest.java`
...
- [ ] T014 [P] Expose and test the defined OpenAPI specifications from `contracts/`
